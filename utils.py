...
class ZkBridge(Help):
    def __init__(self, privatekey, delay, chain, to, api, mode, proxy=None):
        ...
        self.moralisapi = api
        self.nft_address = (
            nfts_addresses[self.nft][self.chain]
            if self.mode == 1
            else reversed_nfts_addresses[self.nft][self.chain]
            if self.mode == 0
            else ""
        )
        ...
        while True:
            try:
                ...
                if response.status_code == 200:
                    ...
                    sign = self.w3.eth.account.sign_message(
                        msg, private_key=self.privatekey
                    )
        ...
        while True:
            try:
                if self.proxy:
                    ...
                    response = requests.post(
                        "https://api.zkbridge.com/api/signin",
                        headers=headers,
                        json=json_data,
                        proxies=proxies,
                    )
        ...
        try:
            if self.proxy:
                ...
                response = session.get(
                    "https://api.zkbridge.com/api/user/profile",
                    params=params,
                    proxies=proxies,
                )
            else:
                response = session.get(
                    "https://api.zkbridge.com/api/user/profile", params=params
                )
            if response.status_code == 200:
                logger.success(
                    f"{self.address}:{self.chain} - успешно авторизовался..."
                )
                return session
        except Exception as e:
            logger.error(f"{self.address}:{self.chain} - {e}")
    ...
    def balance_and_get_id(self):
        if self.chain != "core":
            try:
                ...
                if id_:
                    logger.success(
                        f"{self.address}:{self.chain} - успешно найдена {self.nft} {id_}..."
                    )
                    return id_
            except Exception as e:
                if "list index out of range" in str(e):
                    logger.error(
                        f"{self.address}:{self.chain} - на кошельке отсутсвует {self.nft}..."
                    )
                    return None
                else:
                    logger.error(f"{self.address}:{self.chain} - {e}...")
            ...
            try:
                contract = self.w3.eth.contract(
                    address=self.w3.to_checksum_address(self.nft_address),
                    abi=zk_nft_abi,
                )
                balance = contract.functions.balanceOf(self.address).call()
                if balance > 0:
                    totalSupply = contract.functions.totalSupply().call()
                    id_ = contract.functions.tokensOfOwnerIn(
                        self.address, totalSupply - 100, totalSupply
                    ).call()[0]
                    return id_
                else:
                    logger.error(
        ...
        while True:
            ...
            try:
                if session:
                    ...
                    tx = zkNft.functions.mint().build_transaction(
                        {
                            "from": self.address,
                            "nonce": nonce,
                            "maxFeePerGas": Wei(int(self.w3.eth.gas_price)),
                            "maxPriorityFeePerGas": Wei(
                                int(self.w3.eth.gas_price * 0.8)
                            ),
                        }
                    )
                    ...
                    logger.info(
                        f"{self.address}:{self.chain} - начинаю минт {self.nft}..."
                    )
                    ...
                    if status == 1:
                        ...
                        self.sleep_indicator(
                            random.randint(self.delay[0], self.delay[1])
                        )
                        return session
                    else:
                        logger.info(
                            f"{self.address}:{self.chain} - пробую минт еще раз..."
                        )
                        self.mint()
            except Exception as e:
                error = str(e)
                if "nonce too low" in error or "already known" in error:
                    logger.success(
                        f"{self.address}:{self.chain} - ошибка при минте, пробую еще раз..."
                    )
                    time.sleep(10)
                    self.mint()
                if (
                    "INTERNAL_ERROR: insufficient funds" in error
                    or "insufficient funds for gas * price + value" in error
                ):
                    logger.error(
                    ...
                    logger.error(
                        f"{self.address}:{self.chain} - {self.nft} можно клеймить только один раз!..."
                    )
                    return False
                else:
                    logger.error(f"{self.address}:{self.chain} - {e}...")
    ...
    def bridge_nft(self):
                ...
                if id_:
                    try:
                        ...
                        if self.chain == "bsc" or self.chain == "core":
                            ...
                            tx["gasPrice"] = self.w3.eth.gas_price
                        logger.info(
                            f"{self.address}:{self.chain} - начинаю апрув {self.nft} {id_}..."
                        )
                        sign = self.account.sign_transaction(tx)
                        hash = self.w3.eth.send_raw_transaction(sign.rawTransaction)
                        status = self.check_status_tx(hash)
                        ...
                        if status == 1:
                            ...
                            logger.info(
                                f"{self.address}:{self.chain} - пробую апрув еще раз..."
                            )
                            approve_nft()
                    except Exception as e:
                        error = str(e)
                        if "nonce too low" in error or "already known" in error:
                            logger.info(
                                f"{self.address}:{self.chain} - ошибка при апруве, пробую еще раз..."
                            )
                            approve_nft()
                        if (
                            "INTERNAL_ERROR: insufficient funds" in error
        ...
        def bridge_():
            contract_address = l0_nft_bridge[self.chain]
            print(contract_address)
            bridge = self.w3.eth.contract(
                address=Web3.to_checksum_address(contract_address), abi=bridge_abi_my
            )
            to = stargate_ids[self.to]

            enco = f"0x000100000000000000000000000000000000000000000000000000000000001b7740"
            fee = bridge.functions.estimateFee(
                Web3.to_checksum_address(self.nft_address), id_, to, self.address, enco
            ).call()
            logger.info(
                f"{self.address}:{self.chain} - начинаю бридж {self.nft} {id_}..."
            )
            while True:
                try:
                    nonce = self.w3.eth.get_transaction_count(self.address)
                    ...
                    if status == 1:
                        ...
                        logger.info(
                            f"{self.address}:{self.chain} - пробую бриджить еще раз..."
                        )
                        bridge_()
                except Exception as e:
                    error = str(e)
                    ...
                    if "nonce too low" in error or "already known" in error:
                        logger.info(
                            f"{self.address}:{self.chain} - ошибка при бридже, пробую еще раз..."
                        )
                        bridge_()
                    else:
                        logger.error(f"{self.address}:{self.chain} - {e}")
                ...
                try:
                    if self.proxy:
                        ...
                        response = session.post(
                            "https://api.zkbridge.com/api/bridge/createOrder",
                            json=json_data,
                            proxies=proxies,
                        )
                    else:
            ...
            while True:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.success(
                            f"{self.address} - сгенерирован blob для клейма..."
                        )
        ...
        while True:
            try:
                if self.proxy:
                    proxies = {"http": self.proxy, "https": self.proxy}
                    response = session.post(
                        "https://api.zkbridge.com/api/bridge/claimOrder",
                        json=json_data,
                        proxies=proxies,
                    )
                else:
                    response = session.post(
                        "https://api.zkbridge.com/api/bridge/claimOrder", json=json_data
                    )
                if response.status_code == 200:
    ...
    def check_status_tx2(self, w3, tx_hash):
        logger.info(
            f"{self.address} - жду подтверждения транзакции {scans[self.to]}{w3.to_hex(tx_hash)}..."
        )
        start_time = int(time.time())
        while True:
            current_time = int(time.time())
    ...
    def redeem_nft(self, session=None, hash_=None):
        ...
        def get_order_by_hash():
            logger.info(
                f"{self.address}:{self.to} - пробую делать redeem {self.nft} в сети назначения..."
            )
            while True:
                params = {
                    "depositHash": hash_,
                ...
                try:
                    if self.proxy:
                        proxies = {"http": self.proxy, "https": self.proxy}
                        response = session.get(
                            "https://api.zkbridge.com/api/bridge/getOrderByDepositHashAndChainId",
                            params=params,
                            proxies=proxies,
                        )
                    else:
            ...
            while True:
                try:
                    ...
                    if response.status_code == 200:
                        data_ = json.loads(response.text)
                        logger.success(
                            f"{self.address} - сгенерирован blob для клейма {self.nft}..."
                        )
                        time.sleep(5)
                        return data_, id_, session
        ...
        def claim_again():
            ...
            claim = w3.eth.contract(
                address=Web3.to_checksum_address(nft_claim_addresses[self.to]),
                abi=zk_claim_abi,
            )
            while True:
                data = gen_blob()
                ...
                try:
                    ...
                    tx = claim.functions.validateTransactionProof(
                        cid,
                        block_hash,
                        proof,
                        blob,
                    ).build_transaction(
                        {
                            ...
                            "maxPriorityFeePerGas": Wei(int(w3.eth.gas_price * 0.8)),
                        }
                    )
                    tx["gas"] = self.w3.eth.estimate_gas(tx)
                    ...
                    if status == 1:
                        ...
                        logger.info(
                            f"{self.address}:{self.chain} - пробую клеймить еще раз..."
                        )
                        claim_again()
                    ...
                    if "execution reverted: Block Header is not set" in error:
                        logger.info(
                            f"{address}:{self.to} - {self.to} лагает, пробую еще раз..."
                        )
                        tt = random.randint(20, 60)
                        logger.info(f"{address}:{self.to} - cплю {tt} секунд...")
                        self.sleep_indicator(tt)
                    elif (
                        "nonce too low" in error
                        or "already known" in error
                        or "Message already executed" in error
                    ):
                        logger.success(
                            f"{self.address}:{self.to} - пробую клеймить еще раз..."
                        )
                        claim_again()
                    elif (
                        "INTERNAL_ERROR: insufficient funds" in error
        ...
        while True:
            ...
            try:
                ...
                if status == 1:
                    ...
                    logger.info(
                        f"{self.address}:{self.chain} - пробую клеймить через redeem еще раз..."
                    )
                ...
                if "execution reverted: Block Header is not set" in error:
                    ...
                    logger.success(
                        f"{self.address}:{self.to} - успешно заклеймил {self.nft}..."
                    )
                    self.sleep_indicator(random.randint(self.delay[0], self.delay[1]))
                    order = self.claimOrder(session, id_, block_hash)
                    if order:
...
class ZkMessage(Help):
    ...
    def msg(self, session, contract_msg, msg, from_chain, to_chain, tx_hash):
        ...
        try:
            if self.proxy:
                ...
                response = session.get(
                    "https://api.zkbridge.com/api/user/profile", json=json_data
                )
            if response.status_code == 200:
                logger.success(
                    f"{self.address}:{self.chain} - cообщение подтвержденно..."
                )
                return True
    ...
    def send_msg(self):
        ...
        while True:
            ...
            if lz_status:
                fee = mailer.functions.estimateLzFee(
                    lz_id, self.address, message
                ).call()
                value = fee + zkFee
                logger.info(
                ...
                try:
                    tx = mailer.functions.sendMessage(
                        to_chain_id,
                        dst_address,
                        lz_id,
                        lzdst_address,
                    ...
                    if status == 1:
                        ...
                        logger.info(
                            f"{self.address}:{self.chain} - пробую еще раз отправлять сообщение..."
                        )
                        self.send_msg()
                ...
                logger.info(
                    f"{self.address}:{self.chain} - cплю 30 секунд так как л0 не активен..."
                )
